namespace SimpleSpreedly
{
    ...
    using Spreedly.Net.Helpers.Exceptions;
    ...
    public class SubscriberHelper
    {
        ...
        public Subscriber SubscribeToFreeTrialPlan(Subscriber subscriber, int subscriptionPlanId, bool forceReallow)
        {
            ...
            if (subscribedSubscriber.Status == SpreedlyStatus.UnprocessableEntity)
                throw new UnprocessableEntityException(string.Format("Failed to subscribe subscriber with customer id {0} to plan. Bad entity sent", subscriber.CustomerId), subscribedSubscriber.RawBody);

            if (subscribedSubscriber.Status == SpreedlyStatus.Forbidden)
                throw new ForbiddenActionException(string.Format("Failed to subscribe subscriber with customer id {0} to plan. Forbidden for this plan or subscriber", subscriber.CustomerId), subscribedSubscriber.RawBody);
